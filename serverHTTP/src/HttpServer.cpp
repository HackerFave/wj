...
#define HttpServer_SESSION_PROTECTION( functionName, ... ) \
    auto this_ = this; \
    if ( !this_ || ( contentLength_ < -1 ) || ( waitWrittenByteCount_ < -1 ) ) \
    { \
        qDebug() << QStringLiteral( "HttpServer::Session::" ) + functionName + ": current session this is null"; \
        return __VA_ARGS__; \
    }
...
// Session
HttpServer::Session::Session(const QPointer<QIODevice> &tcpSocket):
    ioDevice_( tcpSocket ),
...
QString HttpServer::Session::requestUrlPath() const
{
    HttpServer_SESSION_PROTECTION( "requestUrlPath", { } );
...
QStringList HttpServer::Session::requestUrlPathSplitToList() const
{
...
QMap< QString, QString > HttpServer::Session::requestUrlQuery() const
{
    const auto indexForQueryStart = requestUrl_.indexOf( "?" );
...
void HttpServer::Session::replyText(const QString &replyData, const int &httpStatusCode)
{
    HttpServer_SESSION_PROTECTION( "replyText" );

    if ( alreadyReply_ )
    {
        qDebug() << "HttpServer::Session::replyText: already reply";
        return;
    }
    ...
    if ( ioDevice_.isNull() )
    {
        qDebug() << "HttpServer::Session::replyText: error1";
        this->deleteLater();
        return;
    }
...
void HttpServer::Session::replyRedirects(const QUrl &targetUrl, const int &httpStatusCode)
{
    HttpServer_SESSION_PROTECTION( "replyRedirects" );

    if ( alreadyReply_ )
    {
        qDebug() << "HttpServer::Session::replyRedirects: already reply";
        return;
    }
    ...
    if ( ioDevice_.isNull() )
    {
        qDebug() << "HttpServer::Session::replyRedirects: error1";
        this->deleteLater();
        return;
    }
...
void HttpServer::Session::replyJsonObject(const QJsonObject &jsonObject, const int &httpStatusCode)
{
    HttpServer_SESSION_PROTECTION( "replyJsonObject" );

    if ( alreadyReply_ )
    {
        qDebug() << "HttpServer::Session::replyJsonObject: already reply";
        return;
    }
    ...
    if ( ioDevice_.isNull() )
    {
        qDebug() << "HttpServer::Session::replyJsonObject: error1";
        this->deleteLater();
        return;
    }
...
void HttpServer::Session::replyJsonArray(const QJsonArray &jsonArray, const int &httpStatusCode)
{
    HttpServer_SESSION_PROTECTION( "replyJsonArray" );

    if ( alreadyReply_ )
    {
        qDebug() << "HttpServer::Session::replyJsonArray: already reply";
        return;
    }
    ...
    if ( ioDevice_.isNull() )
    {
        qDebug() << "HttpServer::Session::replyJsonArray: error1";
        this->deleteLater();
        return;
    }
...
void HttpServer::Session::replyFile(const QString &filePath, const int &httpStatusCode)
{
    HttpServer_SESSION_PROTECTION( "replyFile" );

    if ( alreadyReply_ )
    {
        qDebug() << "HttpServer::Session::replyFile: already reply";
        return;
    }
    ...
    if ( ioDevice_.isNull() )
    {
        qDebug() << "HttpServer::Session::replyFile: error1";
        this->deleteLater();
        return;
    }
    ...
    {
        qDebug() << "HttpServer::Session::replyFile: open file error:" << filePath;
        ioDeviceForReply_.clear();
        this->deleteLater();
        return;
    }
...
void HttpServer::Session::replyImage(const QImage &image, const int &httpStatusCode)
{
    HttpServer_SESSION_PROTECTION( "replyImage" );

    if ( alreadyReply_ )
    {
        qDebug() << "HttpServer::Session::replyImage: already reply";
        return;
    }
    ...
    if ( ioDevice_.isNull() )
    {
        qDebug() << "HttpServer::Session::replyImage: error1";
        this->deleteLater();
        return;
    }
    ...
    {
        qDebug() << "HttpServer::Session::replyImage: open buffer error";
        delete buffer;
        this->deleteLater();
        return;
    }
    ...
    {
        qDebug() << "HttpServer::Session::replyImage: save image to buffer error";
        delete buffer;
        this->deleteLater();
        return;
    }
...
void HttpServer::Session::replyImage(const QString &imageFilePath, const int &httpStatusCode)
{
    HttpServer_SESSION_PROTECTION( "replyImage" );
...
void HttpServer::Session::replyBytes(const QByteArray &bytes, const int &httpStatusCode)
{
    HttpServer_SESSION_PROTECTION( "replyBytes" );
    ...
    if (alreadyReply_)
    {
        qDebug() << "HttpServer::Session::replyBytes: already reply";
        return;
    }
    alreadyReply_ = true;
    ...
    if (ioDevice_.isNull())
    {
        qDebug() << "HttpServer::Session::replyBytes: error1";
        this->deleteLater();
        return;
    }
    ...
    if (!buffer->open(QIODevice::ReadWrite))
    {
        qDebug() << "HttpServer::Session::replyBytes: open buffer error";
        delete buffer;
        this->deleteLater();
        return;
    }
...
void HttpServer::Session::replyOptions()
{
    HttpServer_SESSION_PROTECTION( "replyOptions" );

    if ( alreadyReply_ )
    {
        qDebug() << "HttpServer::Session::replyOptions: already reply";
        return;
    }
    ...
    if ( ioDevice_.isNull() )
    {
        qDebug() << "HttpServer::Session::replyOptions: error1";
        this->deleteLater();
        return;
    }
...
void HttpServer::Session::inspectionBufferSetup1()
{
    if ( !headerAcceptedFinish_ )
    {
        forever
        {
            ...
            if ( splitFlagIndex == -1 )
            {
                ...
                {
                    qDebug() << "HttpServer::Session::inspectionBuffer: error0";
                    this->deleteLater();
                    return;
                }
            ...
            {
                qDebug() << "HttpServer::Session::inspectionBuffer: error1";
                this->deleteLater();
                return;
            }
            ...
            if ( requestMethod_.isEmpty() )
            {
                ...
                {
                    qDebug() << "HttpServer::Session::inspectionBuffer: error2";
                    this->deleteLater();
                    return;
                }
                ...
                if ( ( requestMethod_ != "GET" ) &&
                     ...
                     ( requestMethod_ != "PUT" ) )
                {
                    qDebug() << "HttpServer::Session::inspectionBuffer: error3:" << requestMethod_;
                    this->deleteLater();
                    return;
                }
            }
...
//                qDebug() << buffer_;
            ...
            else
            {
                ...
                if ( index <= 0 )
                {
                    qDebug() << "HttpServer::Session::inspectionBuffer: error4";
                    this->deleteLater();
                    return;
                }
...
void HttpServer::Session::inspectionBufferSetup2()
{
...
// AbstractManage
HttpServer::AbstractManage::AbstractManage(const int &handleMaxThreadCount)
{
...
HttpServer::AbstractManage::~AbstractManage()
{
    this->stopHandleThread();
}

bool HttpServer::AbstractManage::begin()
{
    if ( QThread::currentThread() != this->thread() )
    {
        qDebug() << "HttpServer::Manage::listen: error: listen from other thread";
        return false;
    }

    if ( this->isRunning() )
    {
        qDebug() << "HttpServer::Manage::close: error: already running";
        return false;
    }
...
void HttpServer::AbstractManage::close()
{
    if ( !this->isRunning() )
    {
        qDebug() << "HttpServer::Manage::close: error: not running";
        return;
    }
...
bool HttpServer::AbstractManage::startServerThread()
{
...
void HttpServer::AbstractManage::stopHandleThread()
{
    handleThreadPool_->waitForDone();
}

void HttpServer::AbstractManage::stopServerThread()
{
    serverThreadPool_->waitForDone();
}

void HttpServer::AbstractManage::newSession(const QPointer< Session > &session)
{
    session->setHandleAcceptedCallback( [ this ](const QPointer< HttpServer::Session > &session){ this->handleAccepted( session ); } );
...
void HttpServer::AbstractManage::handleAccepted(const QPointer<Session> &session)
{
    QtConcurrent::run( handleThreadPool_.data(), [ this, session ]()
    {
        if ( !this->httpAcceptedCallback_ )
        {
            qDebug() << "HttpServer::Manage::handleAccepted: error, httpAcceptedCallback_ is nullptr";
            return;
        }
...
// TcpServerManage
HttpServer::TcpServerManage::TcpServerManage(const int &handleMaxThreadCount):
    AbstractManage( handleMaxThreadCount )
{

}

HttpServer::TcpServerManage::~TcpServerManage()
{
    if ( this->isRunning() )
    {
        this->close();
...
bool HttpServer::TcpServerManage::listen(const QHostAddress &address, const quint16 &port)
{
...
bool HttpServer::TcpServerManage::isRunning()
{
    return !tcpServer_.isNull();
}

bool HttpServer::TcpServerManage::onStart()
{
    mutex_.lock();
...
void HttpServer::TcpServerManage::onFinish()
{
    this->mutex_.lock();
...
HttpServer::SslServerManage::SslServerManage(const int &handleMaxThreadCount):
    AbstractManage( handleMaxThreadCount )
{ }

HttpServer::SslServerManage::~SslServerManage()
{
    if ( this->isRunning() )
    {
        this->close();
...
bool HttpServer::SslServerManage::listen(
...
bool HttpServer::SslServerManage::isRunning()
{
    return !tcpServer_.isNull();
}

bool HttpServer::SslServerManage::onStart()
{
    mutex_.lock();
...
void HttpServer::SslServerManage::onFinish()
{
    this->mutex_.lock();
...
// LocalServerManage
HttpServer::LocalServerManage::LocalServerManage(const int &handleMaxThreadCount):
    AbstractManage( handleMaxThreadCount )
{ }

HttpServer::LocalServerManage::~LocalServerManage()
{
    if ( this->isRunning() )
    {
        this->close();
...
bool HttpServer::LocalServerManage::listen(const QString &name)
{
    listenName_ = name;
...
bool HttpServer::LocalServerManage::isRunning()
{
    return !localServer_.isNull();
}

bool HttpServer::LocalServerManage::onStart()
{
    mutex_.lock();
...
void HttpServer::LocalServerManage::onFinish()
{
    this->mutex_.lock();
