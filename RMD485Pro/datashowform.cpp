...
//    ui->frame_2->setVisible(false);
    ...
    _minbmq = publicClass::getInstance()->readValueIni("MIN/duo").toInt()*524287+publicClass::getInstance()->readValueIni("MIN/dan").toInt();
    _maxbmq = publicClass::getInstance()->readValueIni("MAX/duo").toInt()*524287+publicClass::getInstance()->readValueIni("MAX/dan").toInt();
...
//    int32_t valuebody = ui->lineEdit_sudu->text().toInt()*100;
//    uint8_t body[5]={};
//    memmove(body,&valuebody,4);
//    unsigned char pbody = publicClass::getInstance()->cllc_checkSum((unsigned char *)body,/*bytebody.size()*/4);
...
void dataShowForm::slotBmq(int32_t dan,int32_t fbl,int32_t duo)
{
    ...
    if(value<=_minbmq /*abs(value-_minbmq)<=6*/){
        if(!_left){
#if 1
            uint8_t head[5]={};
            head[0] = 0x3e;
    ...
    else if (/*abs(value-_maxbmq)<=6*/value>=_maxbmq) {
        if(!_right){
#if 1
            uint8_t head[5]={};
            head[0] = 0x3e;
...
void dataShowForm::on_pushButton_close_form_clicked()
{
    this->close();
}
...
bool dataShowForm::event(QEvent *event)
{
    if(event->type()==QEvent::NonClientAreaMouseButtonPress){
        qDebug()<<"NonClientAreaMouseButtonPress"<<"111";
    }
    else if (event->type()==QEvent::NonClientAreaMouseButtonRelease) {
        qDebug()<<"NonClientAreaMouseButtonRelease"<<"222";
    }
    else if (event->type()==QEvent::NonClientAreaMouseButtonDblClick) {
        qDebug()<<"NonClientAreaMouseButtonDblClick"<<"333";
    }
    else if (event->type()==QEvent::NonClientAreaMouseMove) {
        qDebug()<<"NonClientAreaMouseMove"<<"444";
    }
    else  {
