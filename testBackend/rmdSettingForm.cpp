...
void rmdSettingForm::slotRecvRmdData_141(UINT id, BYTE *data)
{
...
void rmdSettingForm::slotRecvRmdData_145(UINT id, BYTE *data)
{
    if(QString::number(id,16)==ui->comboBox_id->currentText()){
        ui->textEdit_rmd_data->append(QString("[ID: %1]").arg(QString::number(id,16).toUpper())+ publicClass::getInstance()->Byte_16(data,8).simplified());
        ui->label_num->setText(QString::number(ui->textEdit_rmd_data->document()->lineCount()));
...
void rmdSettingForm::on_pushButton_write_jiasudu_clicked()
{
    ...
    for(int i=0;i<sendMsg.size();i++){
        ...
        ULONG flag =Transmit(3,0,publicClass::getInstance()->readValueIni("index/machineId").simplified().split(":")
                                 .at(ui->comboBox_id->currentIndex()).split("-").at(1).toInt(),&sendbuf[i],1); //调用动态链接库发送函数
        if(flag<1)  //发送不正常
        {
            //             emit signalError("error");
...
void rmdSettingForm::on_pushButton_stop_dianji_clicked()
{
    ...
    for(int i=0;i<sendMsg.size();i++){
        ...
        ULONG flag =Transmit(4,0,publicClass::getInstance()->readValueIni("index/machineId").simplified().split(":")
                             .at(ui->comboBox_id->currentIndex()).split("-").at(1).toInt(),&sendbuf[i],1); //调用动态链接库发送函数
        if(flag<1)  //发送不正常
        {
            //             emit signalError("error");
...
void rmdSettingForm::setValue_141(int32_t valueencoder,int32_t valuespeed)
{
    if("141"==ui->comboBox_id->currentText().simplified()){
        ui->lineEdit_bmq->setText(QString::number(valueencoder));
        ui->lineEdit_dianji_sudu->setText(QString::number(valuespeed));
    }
}
void rmdSettingForm::setValue_145(int32_t valueencoder,int32_t valuespeed)
{
    if("145"==ui->comboBox_id->currentText().simplified()){
        ui->lineEdit_bmq->setText(QString::number(valueencoder));
        ui->lineEdit_dianji_sudu->setText(QString::number(valuespeed));
    }
}
