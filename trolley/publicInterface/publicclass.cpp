...
publicClass::publicClass(QObject *parent ):QObject(parent)
{
    ...
    //iniSystemConfigFile();
    _setting = new QSettings(/*QCoreApplication::applicationDirPath()*/QDir::currentPath()+"/config/config.ini",QSettings::IniFormat);
    qDebug()<<"[PUBLIC PATH:]"<</*QCoreApplication::applicationDirPath()*/QDir::currentPath()+"/config/config.ini";
}
