...
private:
    ...
    purge_workers()
    {
        ...
        for (std::deque<worker *>::iterator wrk = toCull.begin(); wrk != toCull.end(); ++wrk)
        {
            if (m_verbose)
            {
                s_console("I: deleting expired worker: %s", (*wrk)->m_identity.c_str());
            }
            worker_delete(*wrk, 0);
        }
    }
    ...
    worker_process(std::string sender, zmsg *msg)
    {
        ...
        std::string command = (char *)msg->pop_front().data();
        ...
        if (command.compare(MDPW_READY) == 0)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    auto seev = msg->pop_front();
                    std::string service_name((char *)seev.data(), seev.size());
                    //    std::string service_name = (char*)msg->pop_front ().data();
        ...
        else
        {
            if (command.compare(MDPW_REPLY) == 0)
            {
                if (worker_ready)
                {
                    ...
                    if(m_verbose)
                    {
                        s_console("I: sending REPLY to client with %s", wrk->m_service->m_name.c_str());
                        msg->dump();
                    }
                    if(!client.compare(wrk->m_identity))
                    {
                        s_console("I: drop invailed message");
                    }
                    else
                    {
                        msg->send(*m_socket);
                ...
                char *command, std::string option, zmsg *msg)
    {
        ...
        { //  Optional frame after command
            msg->push_front((char *)option.c_str(), option.size());
        }
        msg->push_front(command, 2);
        msg->push_front((char *)MDPW_WORKER, strlen(MDPW_WORKER));
        //  Stack routing envelope to start of message
        msg->wrap(worker->m_identity.c_str(), "");

        // LOG_Debug() << "Send sender: " << mdps_commands[(int)*command] << " header: " << MDPW_WORKER;
        if (m_verbose && (int)*command != 4)
        {
            s_console("I: sending %s to worker", mdps_commands[(int)*command]);
            msg->dump();
        }
        msg->send(*m_socket);
        delete msg;
    ...
    start_brokering()
    {
        ...
        while (!s_interrupted)
        {
            ...
            if (items[0].revents & ZMQ_POLLIN)
            {
                ...
                if (m_verbose && msg->broker_cmd_type() != 4)
                {
                ...
                std::string sender_str((char *)sender.data(), sender.size());
                msg->pop_front(); //empty message
                auto header = msg->pop_front();
                std::string header_str((char *)header.data(), header.size());

                // LOG_Debug() << "Recved sender: " << sender_str << " header: " << header_str;

                if (header_str.compare(MDPC_CLIENT) == 0)
                {
                    client_process(sender_str, msg);
                }
                else if (header_str.compare(MDPW_WORKER) == 0)
                {
                    worker_process(sender_str, msg);
                }
                else
                {
