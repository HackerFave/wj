...
public:
    ...
    zmq_client(std::string broker = MDPC_ADDRESS, int verbose = 0)
    {
        srand(time(NULL));
    ...
    void connect_to_broker()
    {
        ...
        std::string id = s_set_id(*m_client);
        int linger = 0;
    ...
    zmsg * send(std::string service, zmsg *&request_p, int timeout = 2500)
    {
        ...
        request->push_front((char *)service.c_str(), service.size());
        request->push_front((char *)MDPC_CLIENT, strlen(MDPC_CLIENT));
        if (m_verbose)
        {
            s_console("Client I: send request to '%s' service:", service.c_str());
            request->dump();
        }
        ...
        while (retries_left && !s_interrupted)
        {
            ...
            while (!s_interrupted)
            {
                ...
                if (items[0].revents & ZMQ_POLLIN)
                {
                    ...
                    if (m_verbose)
                    {
                        s_console("Client I: received reply:");
                        recv_msg->dump();
                    }
                    //  Don't try to handle errors, just assert noisily
                    assert(recv_msg->parts() >= 3);

                    std::vector<uint8_t> header = recv_msg->pop_front();
                    std::string header_str(header.begin(), header.end());
                    assert(header_str.compare(MDPC_CLIENT) == 0);

                    std::vector<uint8_t> reply_service = recv_msg->pop_front();
                    std::string str_reply(reply_service.begin(), reply_service.end());
                    assert(str_reply.compare(service.c_str()) == 0);
