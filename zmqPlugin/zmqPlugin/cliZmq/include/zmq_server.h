...
public:
    ...
    zmq_server(std::string service, std::string broker = MDPC_ADDRESS, int verbose = 0)
    {
        srand(time(NULL));
    ...
    void send_to_broker(char *command, std::string option, zmsg *_msg)
    {
        ...
        if (option.length() != 0)
        {
            msg->push_front((char *)option.data(), option.length());
        }
        msg->push_front(command, 2);
        msg->push_front((char *)MDPW_WORKER, strlen(MDPW_WORKER));
        msg->push_front((char *)"", 0);

        if (m_verbose && (int)*command != 4)
        {
            s_console("Server I: sending %s to broker", mdps_commands[(int)*command]);
            msg->dump();
        }
        msg->send(*m_worker);
        delete msg;
    ...
    zmsg *recv (zmsg *&reply_p)
    {
        ...
        while (!s_interrupted)
        {
            ...
            if (items[0].revents & ZMQ_POLLIN)
            {
                ...
                if (m_verbose && msg->server_cmd_type() != 4)
                {
                    s_console("Server I: received message from broker:");
                    msg->dump();
                }
                m_liveness = HEARTBEAT_LIVENESS;
                ...
                std::vector<uint8_t> empty = msg->pop_front();
                std::string empty_str(empty.begin(), empty.end());
                assert(empty_str.compare("") == 0);

                std::vector<uint8_t> header = msg->pop_front();
                std::string header_str(header.begin(), header.end());
                assert(header_str.compare(MDPW_WORKER) == 0);
