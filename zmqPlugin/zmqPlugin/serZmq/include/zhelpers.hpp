...
static uint64_t _random()
{
    ...
    _rand_value = *(uint32_t*)pbBuffer;
    ::CryptReleaseContext(hProvider, 0);
    return _rand_value;
}
#define within(num) (long) ((uint64_t)((num) * (uint64_t)_random ()) / (65535 + 1.0))
#else
#define within(num) (long) ((uint64_t)((num) * (uint64_t)random ()) / (RAND_MAX + 1.0))
#endif
